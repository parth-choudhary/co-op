...
export default function DashboardPage() {
  ...
  const stats = [
    ...
    { label: 'Active Tasks', value: '0', icon: CheckCircle2, color: 'var(--color-success-light)' },
    { label: 'Messages', value: '0', icon: MessageSquare, color: 'var(--color-role-cmo)' },
  ];
  const quickActions = [
    { title: 'Create a Project', description: 'Start a new project with a Kanban board', href: '/projects', icon: FolderKanban, color: 'var(--color-accent)' },
    { title: 'Add an AI Agent', description: 'Hire a CTO, CMO, or custom AI role', href: '/agents', icon: Bot, color: 'var(--color-role-cto)' },
    { title: 'Setup API Keys', description: 'Connect OpenAI or Anthropic', href: '/settings', icon: Sparkles, color: 'var(--color-info)' },
  ];
    ...
    <div style={{ padding: 'var(--space-8)', maxWidth: 1200 }}>
      <div className="glass-card" style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: 'var(--space-8)', marginBottom: 'var(--space-6)', position: 'relative', overflow: 'hidden' }}>
        <div style={{ position: 'relative', zIndex: 1 }}>
          <h1 className="heading-2">Welcome back, <span style={{ color: 'var(--color-accent)' }}>{user?.name || 'there'}</span></h1>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(200px, 1fr))', gap: 'var(--space-4)', marginBottom: 'var(--space-8)' }}>
        {stats.map((s) => (
          <div key={s.label} className="glass-card" style={{ display: 'flex', alignItems: 'center', gap: 'var(--space-4)', padding: 'var(--space-5)' }}>
            <div style={{ width: 44, height: 44, display: 'flex', alignItems: 'center', justifyContent: 'center', background: 'var(--color-bg-tertiary)', border: '1px solid var(--color-surface-border)', borderRadius: 'var(--radius-lg)', color: s.color }}><s.icon size={22} /></div>
            <div><div style={{ fontSize: 'var(--font-size-2xl)', fontWeight: 'var(--font-weight-bold)', lineHeight: 1, fontFamily: 'var(--font-family-heading)' }}>{s.value}</div><div className="text-tertiary text-sm" style={{ marginTop: 'var(--space-1)' }}>{s.label}</div></div>
          </div>
        ))}
      </div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(300px, 1fr))', gap: 'var(--space-4)', marginBottom: 'var(--space-8)' }}>
        {quickActions.map((a) => (
          <Link key={a.title} href={a.href} className="glass-card" style={{ display: 'flex', alignItems: 'center', gap: 'var(--space-4)', padding: 'var(--space-5)', textDecoration: 'none' }}>
            <div style={{ width: 48, height: 48, display: 'flex', alignItems: 'center', justifyContent: 'center', borderRadius: 'var(--radius-lg)', color: a.color, background: 'var(--color-bg-tertiary)', border: '1px solid var(--color-surface-border)', flexShrink: 0 }}><a.icon size={24} /></div>
            <div style={{ flex: 1 }}><div style={{ fontWeight: 'var(--font-weight-semibold)', color: 'var(--color-text-primary)' }}>{a.title}</div><div className="text-tertiary text-sm" style={{ marginTop: 'var(--space-1)' }}>{a.description}</div></div>
      ...
      <div className="glass-card" style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 'var(--space-3)', padding: 'var(--space-10)', textAlign: 'center' }}>
        <Sparkles size={32} style={{ color: 'var(--color-accent)', opacity: 0.4 }} />
